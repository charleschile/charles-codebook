...
For our risc game, we choose to use the traditional java Swing to build our frontend.

Java Swing is a set of graphical user interface components for java programs, it's part of the java foundation classes, so it can run on any platform that supports java.

In our program we used Swing components like - JFrame (main window), JPanel (components within the window), JDialog (error messages)

Also many of our classes implement ActionListener interface to handle the user interactions with our GUI components like clicking buttons.
