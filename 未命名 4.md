...
I don't remember. But then, global variable is relatively straightforward.

And the output can be Json.


So once you pass the model name as an argument to the create index. We will be using the model name, and then basically mapping corresponding dimension length. And that will be used for creating the underlying tables. Okay.
...
Hmm, yeah. This would be the part where you create. So this would be the syntax. Okay, create Llm index. Idx on table text
